...
export function MagicEquipmentBadges({ item }: { item: Equipment }) {
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-1">
      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-magenta/10 text-magenta font-semibold uppercase tracking-wide">
        Magic
      </span>
      {item.rarity ? (
      ...
      {item.requires_attunement ? (
        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-orange/10 text-orange font-medium">
          Attunement
        </span>
      ) : null}
    </span>
