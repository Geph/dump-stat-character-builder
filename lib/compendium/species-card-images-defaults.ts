...
export const SPECIES_CARD_IMAGES_BY_NAME: Record<string, string> = {
  ...
  Farling: speciesCardImage("farling"),
  ...
  Ironwrought: speciesCardImage("ironwrought"),
...
export function defaultSpeciesCardImageUrl(
  speciesName: string,
  ...
  const accept = (url: string | null) => maybeFilterDefaultCardImageUrl(url, requireAvailable)

  const exact = lookupSpeciesCardImage(trimmed)
  if (exact) return accept(exact)
  ...
  if (colonMatch) {
    ...
    if (left && right) {
      ...
      if (flipped) return accept(flipped)
    }
  }
    ...
    if (base) {
      ...
      if (year === "2014") {
        ...
        if (!lookupSpeciesCardImage(`${base} (2024)`)) {
          ...
          if (baseKeyed) return accept(baseKeyed)
        }
      } else if (year === "2024") {
