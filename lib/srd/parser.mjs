...
/** Keep choice phrases intact; split only concrete comma lists. */
function parseClassToolProficiencies(raw) {
  if (!raw?.trim()) return []
  const cleaned = raw.trim().replace(/\s*\(see\s+[^)]+\)\s*$/i, "").trim()
  if (!cleaned || /^none$/i.test(cleaned)) return []
  if (/\b(?:choose|select)\b/i.test(cleaned) || /\bor\b/i.test(cleaned)) {
    return [cleaned]
  }
  return cleaned
    .split(/,|\band\b/gi)
...
export function parseClasses(md) {
  ...
  for (const { title: className, body } of splitSections(md, 2)) {
    ...
    const toolsRaw = getTrait(traits, "Tool Proficiencies") ?? getTrait(traits, "Tool Proficiency")
    ...
    const toolProficiencies = parseClassToolProficiencies(toolsRaw)
...
/**
 * SRD 5.2.1 markdown nests the Adventuring Gear catalog table under #### Ammunition.
 * Drop that catalog from item prose, but keep any later item-specific tables (e.g. ammo types).
 */
function cleanGearDescriptionBody(raw) {
  let body = String(raw ?? "")
  body = body.replace(/\*\*Adventuring Gear\*\*\s*<table>[\s\S]*?<\/table>/i, "\n")
  // Subtype packs are imported as their own gear rows.
  body = body.replace(/\*\*Ammunition\*\*\s*<table>[\s\S]*?<\/table>/i, "\n")
  return stripHtml(body).replace(/\s+/g, " ").trim().slice(0, 4000)
}
...
function parseGearDescriptions(md) {
  ...
  while ((m = re.exec(block))) {
    ...
    const description = cleanGearDescriptionBody(m[2])
    if (name) {
      descriptions.set(name.toLowerCase(), {
...
/** Individual ammo packs from the nested Ammunition subtype table. */
function parseAmmunitionSubtypeItems(md) {
  const gearBlock = md.split("## Adventuring Gear")[1]?.split("## Mounts")[0] ?? ""
  const match = gearBlock.match(/\*\*Ammunition\*\*\s*(<table>[\s\S]*?<\/table>)/i)
  if (!match) return []

  const rows = parseHtmlTableRows(match[1])
  const items = []
  for (const cells of rows) {
    if (cells.length < 5 || /^(type|amount)$/i.test(cells[0])) continue
    const [type, amount, storage, weight, cost] = cells
    const name = stripHtml(type)
    if (!name) continue
    const amountLabel = stripHtml(amount)
    const storageLabel = stripHtml(storage)
    items.push({
      name,
      ...
      description: [
        amountLabel ? `${amountLabel} ${name}.` : null,
        storageLabel && storageLabel !== "—"
          ? `Typically stored in a ${storageLabel} (bought separately).`
          : null,
      ]
        .filter(Boolean)
        .join(" "),
      properties: {
        ...
        storage: storageLabel && storageLabel !== "—" ? storageLabel : null,
      },
      source: SOURCE,
    })
  }
...
export function parseEquipment(md) {
  const equipment = [
    ...
    ...parseToolsTable(md),
    ...parseGearTable(md),
    ...parseAmmunitionSubtypeItems(md),
  ]
