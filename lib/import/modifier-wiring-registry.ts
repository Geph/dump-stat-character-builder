...
function formatMechanicsCheatsheet(): string {
  const lines = [
    ...
    "- check_roll_modifier: checkRollMode advantage|disadvantage|bonus; checkCategory save|skill|ability|attack|initiative|death_save; checkAbility/checkSkills; conditionNote for non-enforcible qualifiers (\"that involves you dancing\") — when conditionNote is set the mechanic is NOT auto-wired (manual review) so imports do not silently over-grant; for Death Saving Throws use checkCategory death_save (not save + conditionNote). Prefer requiresSheetToggle (or a free-text label) instead of conditionNote when the gate is player-tracked (\"after drinking a healing potion\", \"to conceal this ring\") — Dump Stat enrichment wires those with optional sheet toggles. for class-resource die bonuses use bonusDiceFromResource / classResourceKey + targets self|self_and_allies_in_range|chosen_creatures",
