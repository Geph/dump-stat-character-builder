...
- Do NOT paste HTML level tables, Core Traits tables, or "Becoming a [Class]" / multiclass checklists into classes[].description — put a short original flavor summary only; features go in features[] and pools (Ki, Rage, Sorcery Points, etc.) in class_resources[]
...
- Preserve HTML <table> blocks for subclass spell lists in feature descriptions when present. If the source text lost its whitespace/line breaks (common with PDF copy-paste, e.g. "Cleric LevelPrepared Spells3Aid, Bless, Cure Wounds5Mass Healing Word..." with no gap between the level number and the spell names), reconstruct it into a real <table> with one <tr> per class level rather than pasting the squished run of text — the importer cannot reliably re-split a level number from the spell name without a delimiter
- Mutually exclusive spell-set choices (Circle of the Land land types, Investigator Thesis Corpus/Mentis/Mortis/Oculus, similar subtype tables): emit isChoice with one option per subject and put THAT subject's HTML spell table in the option description — never mash every table into the parent feature alone. Level-up-only swaps (Thesis) use swappableOnRest: false; rest swaps use swappableOnRest: true
...
const CONTENT_TYPE_JSON_FOCUS: Partial<Record<ImportContentTypeHint, string>> = {
  classes:
    "Focus on classes[] and class_resources[] when present. Always include that class's subclasses/archetypes/paths in subclasses[] in the same JSON (class_name must match classes[].name). Use top-level armor_proficiencies, weapon_proficiencies, and tool_proficiencies arrays — never nest them under a proficiencies object (that field is ignored on classes). Put fixed tools (e.g. \"Thieves' Tools\", \"Herbalism Kit\") and choice phrasing (e.g. \"Choose 3 Musical Instruments\", \"Choose one type of Artisan's Tools or Musical Instrument\", \"Choose one kind of Artisan's Tools\") in tool_proficiencies — Dump Stat turns choice phrases into tool pickers. Include hit_die, complexity (easy/medium/hard), saving_throws, and skill_choices from the Skills line (count + options; use fixed for always-granted skills like Psionics). Put a short original flavor summary in description (\"How it feels to play\") — not the level progression table, Core Traits table, or \"Becoming a [Class]\" / multiclass checklists (features and resource columns are extracted separately). Archetype unlock features (Psionic Archetype, Martial Archetype, etc.) are short unlock prose only — do NOT put isChoice stub options listing archetype names; real archetypes go in subclasses[]. Secondary/Third Discipline (Psion) must be isChoice with choices.category \"Psionic Discipline\" and empty options[] (optionsSource is filled on import). Psionic Talents uses choices.category \"Psionic Talent\" and empty options[] — do not set optionsSource to class_talents. Wire Common Modifiers via description phrasing and optional mechanics[] on each feature. If the source includes a dedicated class spell list (Spell / School / Special tables), also populate that class's spell_list and matching spells[] stubs in the same JSON (school string required — use \"Unknown\" when the list has no school column; concentration boolean required — default false). When starting gear or features introduce a non-SRD weapon, also emit a full equipment[] row in the same JSON (damage, properties.properties tags, mastery, cost, weight, and new-property rule text in description). Listing only the name in starting_equipment_groups leaves the weapon unmatched and flagged on import.",
  subclasses:
