...
describe("enrichPsionArchetypeFeatures", () => {
  it("wires real-time recharge on key archetype features", () => {
    ...
    const enriched = enrichPsionArchetypeFeatures(content)
    const knowing = enriched.subclasses?.[0]
    const climactic = knowing?.features?.find((f) => f.name === "Climactic Moment")
    ...
    const shattered = knowing?.features?.find((f) => f.name === "Shattered Husks")
    ...
    const planeswalker = enriched.subclasses?.[1]?.features?.find((f) => f.name === "Planeswalker")
