...
export function classifyImportAiError(error: unknown): ClassifiedImportAiError {
  ...
  if (
    ...
    return {
      ...
      message,
      userMessage:
        "Import provider quota exceeded. Use BYO LLM import with your own LLM, or switch provider/model in server AI settings.",
      retryable: true,
      status: 429,
    }
  }
  ...
  if (
    ...
    return {
      ...
      message,
      userMessage:
        "The selected import model is not available on this provider. Pick another model in server AI settings, or use BYO LLM import.",
      retryable: false,
      status: 400,
    }
  }
