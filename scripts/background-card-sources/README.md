...
Source files here are gitignored. Commit optimized outputs only from `SRD/`. Kibbles, Mage Hand Press, `PHB/`, `Eberron/`, `Ravenloft-Astarion/`, and `Planescape/` stay on your machine — import still uses them when those PNGs exist locally after `pnpm images:optimize`.
